...
int main(int argc, char **argv) {
  ...
  if ((bind(socketServ, (const struct sockaddr *)&servaddr,
            sizeof(servaddr))) != 0)
    error_exit("Fatal error\n", NULL, socketServ);
  ...
  while (1) {
    ...
    if (select(max + 1, &rfds, &wfds, NULL, 0) < 0)
      continue;

    if (FD_ISSET(socketServ, &rfds)) {

      int newFd = accept(socketServ, (struct sockaddr *)&cliaddr, &len);
      if (newFd < 0)
        error_exit("Fatal error\n", clients, socketServ);
      ...
      newCli->id = nbClients;
      nbClients++;
      memset(newCli->buffer, 0, sizeof(newCli->buffer));
      newCli->msg = NULL;
      newCli->next = NULL;
    ...
    while (cli) {
      ...
      if (FD_ISSET(cli->fd, &wfds) && cli->msg) {
        message *tmp = cli->msg;
        int blop = send(cli->fd, tmp->content + tmp->offset,
                        strlen(tmp->content) - tmp->offset, 0);
        if (blop > 0) {
